...
import { getFirecrawlService } from '../src/lib/firecrawl.ts';
    ...
    for (const source of sources) {
      ...
      try {
        ...
        if (source.type === 'scrape') {
          ...
          const scrapedData = await firecrawl.scrapeJobListing(source.url);
          jobs = scrapedData ? [scrapedData] : [];
        }
        ...
        for (let i = 0; i < jobs.length; i++) {
          ...
          try {
            ...
            // Check if the job already exists in the database, and get its manual_education_level
            const { data: existingJobs, error: checkError } = await supabase
              .from('jobs')
              .select('id, manual_education_level')
              .eq('title', job.title)
            ...
            if (existingJobs && existingJobs.length > 0) {
              ...
              // Use manual_education_level if it exists, otherwise use LLM classification
              const finalEducationLevel = existingJobs[0].manual_education_level || educationLevel;
              ...
              if (updateError) {
                ...
                console.log(`Updated job ${existingJobs[0].id} with education level: ${finalEducationLevel}, time commitment: ${timeCommitment || 'Unknown'}`);
              }
            } else {
              ...
              const { error: insertError } = await supabase
                ...
                .insert({
                  ...
                  education_level: educationLevel, // For new jobs, use LLM classification initially
                  time_commitment: timeCommitment,
