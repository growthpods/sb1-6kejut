    ...
    try {
        ...
        for (let i = 0; i < jobs.length; i++) {
            ...
            try {
                ...
                const manualOverride = job.manual_education_level;
                let studentType;
                if (manualOverride && (manualOverride === 'High School' || manualOverride === 'College')) {
                    console.log(`Manual override found for job '${job.title}': ${manualOverride}`);
                    studentType = manualOverride;
                }
                else {
                    // Determine whether the job is suitable for high school or college students
                    studentType = await educationLevelParser.parseEducationLevelFromText(job.title || '', job.description || '', job.requirements || [], null // no manual override
                    );
                }
                if (!studentType) {
                    // Log for manual review
                    console.warn(`Job '${job.title}' (ID: ${job.id}) could not be classified by AI. Needs manual review.`);
                }
