// Simple script to directly add the source and career_site_url columns to the jobs table
...
async function updateJobsTable() {
  console.log('Starting direct update of jobs table to add source and career_site_url fields...');
  ...
  try {
    ...
    if (beforeData.length > 0) {
      ...
      const sourceExists = 'source' in firstJob;
      const careerSiteUrlExists = 'career_site_url' in firstJob;
      
      if (sourceExists && careerSiteUrlExists) {
    ...
    for (const job of allJobs) {
      const { error: updateError } = await supabase
        .from('jobs')
        .update({
          source: 'Manual', // Default value for existing jobs
          career_site_url: null // Default to null
        })
        .eq('id', job.id);
    ...
    if (afterData.length > 0) {
      ...
      const sourceExists = 'source' in updatedJob;
      const careerSiteUrlExists = 'career_site_url' in updatedJob;
      
      console.log(`source field: ${sourceExists ? '✅ exists' : '❌ missing'}`);
      console.log(`career_site_url field: ${careerSiteUrlExists ? '✅ exists' : '❌ missing'}`);
    }
