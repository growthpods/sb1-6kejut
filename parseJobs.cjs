...
async function main() {
  ...
  const { getEducationLevelParser } = await import('./src/lib/educationLevelParser.js');
  const { getTimeCommitmentParser } = await import('./src/lib/timeCommitmentParser.js');
  const { getFirecrawlService } = await import('./src/lib/firecrawl.js');
